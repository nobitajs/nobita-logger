...
module.exports = config => {
  ...
  const src = path.join(config.cwd, logger.path);
  
  log4js.configure({
    appenders: {
      file: {
        type: 'file',
        filename: `${src}${formatDateTime()}.log`,//文件目录，当目录文件或文件夹不存在时，会自动创建
    ...
    categories: {
      default: { appenders: ['file', 'dateFile', 'out'], level: logger.level }
    }
  });
